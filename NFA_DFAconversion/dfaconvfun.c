...
void input()
{
	...
	disp(tt,states,p);
	if(lambda==1)
		convert_lam(tt,states,n,fin,nfinal);
	else 
		convert(tt,states,n,fin,nfinal);
	free(tt);
}
void lmd(char **tt,int states,int n)
{
	//Taking input if there are lambda transitions
	int i,j,p;
	int prod=n*states;
	for(i=0;i<states;i++)
		{
...
{
	//Checking if the input has a lambda transition or not
	int j,i,prod,x=0;
	prod=(n-1)*states;
		if(t==-1)
		{
...
void convert(char **tt1,int states,int n, int * final, int nfinal)
{
	//Conversion when there is no lambda transitions
	int i,j;
	struct node *head;
	head=NULL;
	int p;
	...
	while(c<n)
	{
		...
		{
			temp1[0]='1';
			memset(temp1+1,'0',states*sizeof(char));
			strcpy((temp1+states),tt1[0]);//
			printf("\n");
	...
	disp1(head,states,n,final, nfinal,0);
...
int disp1(struct node * h, int states, int n, int *final, int nfinal, int lam)
{
	int i=0;
	int j,k=0;
	int key;
...
void convert_lam(char **tt1,int states,int n, int *final ,int nfinal)
{
	//Conversion when there are lambda transitions
	int i,j,k,k1;
	struct node *head;
	head=NULL;
	int p;
	...
	disp1(head,states,n,final,nfinal,1);
